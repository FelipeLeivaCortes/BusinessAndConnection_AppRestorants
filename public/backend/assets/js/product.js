     ...
     $(document).on('click','#generate_variations',function(e){
		...
		$('.variable-product .product_option').each(function(index, value ) {
            if($(this).val() == ''){
            	...
            	$(this).css('border','1px solid #c7def7');
            }
		});
		...
		$.ajax({
			url: _user_url + '/products/generate_variations',
			method: 'POST',
			data: { 
			...
			success: function(data){
				...
				$.each(json,function(index, value){
					...
					column += '<td><input type="text" name="variation_price[]" class="form-control" value="'+ $('input[name="price"]').val() +'" placeholder="' + $lang_price + '" required></td>';
					column += '<td><input type="text" name="variation_special_price[]" class="form-control" value="'+ $('input[name="special_price"]').val() +'" placeholder="' + $lang_special_price + '"></td>';
