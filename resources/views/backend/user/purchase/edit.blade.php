						...
						<div class="col-12">
							<div class="form-group">					
								<select class="form-control" id="products" data-type="purchase" data-value="id" data-display="name" data-placeholder="{{ _lang('Select an Item') }}" data-modal="ajax-modal"
									data-href="{{ route('purchase_items.create') }}" data-title="{{ _lang('Add New Item') }}" data-table="purchase_products" data-where="3">
								</select>
							</div>
						</div>
					</div>
							...
							<div class="table-responsive">
								<table class="table" id="invoice-table">
									...
									<tbody>
										...
										<tr class="line-item">	
											<td class="input-lg">
												<input type="hidden" class="product_id" name="product_id[]" value="{{ $item->purchase_product_id }}">
											...
											<td class="input-xs text-center">
												<input type="number" class="form-control quantity" name="quantity[]" value="{{ $item->quantity }}" min="1" {{ $item->product->stock_management == 1 ? 'max=' . $item->product->stock + $item->quantity : '' }}>
											</td>
			...
			<div class="card sticky-card">
				<div class="card-body">
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label class="control-label">{{ _lang('Taxes') }}</label>						
								<select id="taxes" name="taxes[]" class="multi-selector select_taxes auto-multiple-select" data-selected="{{ $purchase->taxes->pluck('tax_id') }}" data-placeholder="{{ _lang('Select Taxes') }}" multiple>
