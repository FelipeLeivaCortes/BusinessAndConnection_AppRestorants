(function ($) {
    ...
    $(document).on("change", "#products", function (e) {
        ...
        $.ajax({
            url: _user_url + "/purchase_items/get_product/" + productId,
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                if (json["product"]["stock_management"] == "1" && selectType != 'purchase') {
                    var available_quantity = parseFloat(json["product"]["stock"]);
                ...
                $(row).find(".product_taxes").attr("name", `taxes[${json["product"]["id"]}][]`);
    ...
    //Select Tax
    $(document).on("change", "#taxes", function (event) {
        var elem = $(this);
        $(".tax-field").remove();

        $.each($(elem).val(), function (index, value) {
            var tax_name = $(elem)
            ...
            if (!$("#tax-" + value).length) {
                $("#after-tax").before(`<div class="form-group row tax-field">
    ...
    $.fn.calculateTotal = function () {
        ...
        if ($("#discount_type").val() == "0") {
            discountAmount = (subTotal / 100) * parseFloat($("#discount_value").val());
        } else if ($("#discount_type").val() == "1") {
            discountAmount = parseFloat($("#discount_value").val());
        }

        //Calculate Taxes
        $.each($("#taxes").val(), function (index, value) {
            var tax_rate = $("#taxes").find('option[value="' + value + '"]').data("tax-rate");
            var tax_amount = (subTotal / 100) * tax_rate;

            if ($("#tax-" + value).length) {
                var existingTaxAmount = parseFloat($("#tax-" + value).val());
                var newTaxAmount = existingTaxAmount + tax_amount;
