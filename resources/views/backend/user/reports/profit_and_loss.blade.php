						...
						<div class="col-md-6 pr-md-0">
							<table class="table">
								...
								@if(isset($report_data))
									...
									<tr>
										<td>{{ _lang('Sales') }}</td>
										<td class="text-right">{{ formatAmount($sales->sub_total, currency_symbol($currency)) }}</td>							
									</tr>
									@php $total_income += $sales->sub_total; @endphp

									@if($sales->service_charge > 0)
										<tr>
											<td>{{ _lang('Service Charge') }}</td>
											<td class="text-right">{{ formatAmount($sales->service_charge, currency_symbol($currency)) }}</td>							
										</tr>
										@php $total_income += $sales->service_charge; @endphp
									@endif

									@if($sales->discount > 0)
										<tr>
											<td>{{ _lang('Sales Discount') }}</td>
											<td class="text-right">-{{ formatAmount($sales->discount, currency_symbol($currency)) }}</td>							
										</tr>
										@php $total_income -= $sales->discount; @endphp
									@endif
								@endif
								</tbody>
							</table>
						...
						<div class="col-md-6 pl-md-0">
							<table class="table">
								...
								@if(isset($report_data))
									...
									<tr>
										<td>{{ _lang('Purchase') }}</td>
										<td class="text-right">{{ formatAmount($purchase_amount, currency_symbol($currency)) }}</td>							
									</tr>
									@php $total_expense += $purchase_amount; @endphp
									@endif
									...
									@foreach($othersExpense as $otherExpense)
										<tr>
											<td>{{ $otherExpense->transaction_category->name }}</td>
									...
									@if($payroll->staff_salary > 0)
									<tr>
										<td>{{ _lang('Staff Salary') }}</td>
										<td class="text-right">{{ formatAmount($payroll->staff_salary, currency_symbol($currency)) }}</td>							
									</tr>
									@php $total_expense += $payroll->staff_salary; @endphp
									@endif

									@if($payroll->expense_claim > 0)
									<tr>
										<td>{{ _lang('Staff Expenses') }}</td>
										<td class="text-right">-{{ formatAmount($payroll->expense_claim, currency_symbol($currency)) }}</td>							
									</tr>
									@php $total_expense -= $payroll->expense_claim; @endphp
									@endif
								@endif
								</tbody>
							</table>
